...
from .helpers import (
    ...
    disable_field,
    generar_opciones_mensual,
    generate_custom_week_calendar,
...
class BaseRequestForm(forms.ModelForm):
    ...
    cronograma_semanal = forms.ChoiceField(
        ...
        choices=generate_custom_week_calendar(datetime.date.today().year),
        required=False,
    )
    cronograma_mensual = forms.ChoiceField(
        ...
        choices=generar_opciones_mensual(datetime.date.today().year),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "codigo_compania" in self.fields:
            disable_field(self.fields["codigo_compania"], "0744")
            logger.debug("Campo 'codigo_compania' deshabilitado y establecido a '0744'")
