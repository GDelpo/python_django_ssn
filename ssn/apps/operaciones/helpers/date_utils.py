...
def generate_custom_week_calendar(year: int) -> Tuple[List[str], ...]:
    """
    ...
    Example:
        >>> calendar = generate_custom_week_calendar(2025)
        >>> print(calendar[0])
...
def generar_opciones_mensual(anio):
    """
    Genera las opciones de mes para un año dado usando la configuración de idioma de Django.
    """
    # Usar el idioma definido en Django
    idioma = settings.LANGUAGE_CODE
    # Mapeo de códigos de idioma de Django a códigos de locale
    locale_mapping = {
        "es-es": "es_ES.UTF-8",  # Para Linux/Unix
        "es": "es_ES.UTF-8",
        # Para Windows puedes añadir:
        # 'es-es': 'Spanish_Spain',
        # 'es-ar': 'Spanish_Argentina'
    }

    # Intentar establecer el locale basado en la configuración de Django
    try:
        locale_code = locale_mapping.get(idioma, "es_ES.UTF-8")
        locale.setlocale(locale.LC_TIME, locale_code)
    except locale.Error:
        # Si falla, intentar con otra alternativa
        try:
            # Alternativa para Windows
            if "es" in idioma:
                locale.setlocale(locale.LC_TIME, "Spanish_Spain")
        except locale.Error:
            # Si sigue fallando, dejar el locale por defecto
            pass
    ...
    for mes in range(1, 13):
        ...
        label = datetime.date(anio, mes, 1).strftime("%B %Y")
